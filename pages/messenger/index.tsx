...
function Messenger(props: MessengerProps) {
  ...
  async function getRoomInfo(roomId: number) {
    ...
    users.forEach((user: Paticipant) => {
      user.self = user.id === session?.user.id
    })
    setUsers(users)
  }
  ...
  useEffect(() => {
    if (router.query.roomId && status === 'authenticated') {
      ...
      s.emit('auth:connect', session?.accessToken)
...
Messenger.Layout = EmptyLayout
Messenger.isPrivate = true
